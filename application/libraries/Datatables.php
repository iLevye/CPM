  ...
  */
  class Datatables
  {
    ...
    */  
    ...
    /**
    * last query function for debug query
    * Emre Can
    */  
    ...
    */
    protected function get_filtering()
    {
      ...
      $sWhere = '';
      $sSearch = $this->ci->db->escape_str($this->ci->input->get('sSearch'));

      //$mColArray = array_values(array_diff($mColArray, array()));
      //$columns = array_values(array_diff($this->columns, array()));
      

      $mColArray = array_values(array_diff($mColArray, $this->unset_columns));
      $columns = array_values(array_diff($this->columns, $this->unset_columns));

      $mColArray = array_merge($mColArray, array_values($this->unset_columns));
      $columns = array_merge($this->columns, array_values($this->unset_columns));
      ...
      if($sSearch != '')
        for($i = 0; $i < count($mColArray); $i++)
          if($this->ci->input->get('bSearchable_' . $i) != 'false' && in_array($mColArray[$i], $columns))
      ...
      {
        if(isset($_GET['sSearch_' . $i]) && $this->ci->input->get('sSearch_' . $i) != '' && in_array($mColArray[$i], $columns))
        {
